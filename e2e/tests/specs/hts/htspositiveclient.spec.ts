...
test("Fill HTS Initial, Retest and Linkage Forms", async ({ page, api }) => {
  ...
  await test.step("Then I should see the newly filled form in the encounters table", async () => {
    await expect(
      page.getByRole("tab", { name: /Visits/i })
    ).toBeVisible();
    await expect(
  ...
  await test.step("When I click the `HTS Retest Form` link to launch the form", async () => {
    await page.getByText(/HTS Retest Form/i).click();
  });

  await test.step("Then I should see the `HTS Retest Form` form launch in the workspace", async () => {
    await expect(page.getByText(/HTS Retest Form/i)).toBeVisible();
  });
  await test.step("Selecting the HTS recording section", async () => {
    await page.getByText(/HTS Recordings/i).first().click();
  })
  ...
  await test.step("Then I should see the newly filled form in the encounters table", async () => {
    ...
    await expect(page.getByText("HIV test date").first()).toBeVisible();
    await expect(page.getByText("Population type").first()).toBeVisible();
  });
  ...
  await test.step("Then I should see the clinical forms workspace", async () => {
    ...
    await expect(
      page.getByRole("cell", { name: "HTS Linkage Form", exact: true })
    ).toBeVisible();
  });

  await test.step("When I click the `HTS Retest Form` link to launch the form", async () => {
    await page.getByText(/HTS Linkage Form/i).click();
  });

  await test.step("Then I should see the `HTS Linkage Form` form launch in the workspace", async () => {
    await expect(page.getByText(/HTS Linkage Form/i)).toBeVisible();
  });
  await test.step("Select referral to this facility ", async () => {
    await page.locator('#referraltoid').selectOption({ value: '163266AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA' });
    const selectedValue = await page.$eval('#referraltoid', e => e.value);
    expect(selectedValue).toContain('163266AAAAAAAAAAAAAAAAAAAAAAAAAAAAAA');
  ...
  await test.step("Fill the  CCC Number", async () => {
    await page.locator('#cccnumberid').fill(Number(Math.floor(Math.random() * (9999999999 - 1000000000 + 1)) + 1000000000).toString());
  })
  await test.step("Fill health worker assigned", async () => {
    await page.locator('#healthcareproviderid').fill('James Test');

  })
  await test.step('Fill cadre of  health Worker', async () => {

    await page.locator('#hcwcadreid').selectOption({ value: '1577AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA' });
    const selectedValue = await page.$eval('#hcwcadreid', e => e.value);
    expect(selectedValue).toContain('1577AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA');
    ...
    await expect(options).toEqual(
      expect.arrayContaining([
        ...
        '1574AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA',
        '1555AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA',
        '1540AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA',
  ...
  await test.step('Fill date entrolled', async () => {
    await page.locator('#dateenrolledid').fill('11/28/2024');
  })
  await test.step("And I click on the `Save` button", async () => {
  ...
  await test.step("Then I should see the newly filled form in the encounters table", async () => {
    ...
    await expect(page.getByText("Type of facility referred from")).toBeVisible();
    await expect(page.getByText("Name of health care provider")).toBeVisible();
  });
});
