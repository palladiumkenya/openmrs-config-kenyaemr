...
let visit: Visit;
const testData = [
  {  gender:"F",birthdate:"1977-11-01",patient:"Patient1" },
  {  gender:"F", birthdate:"1975-11-12" ,patient:"Patient2"},
...
test(`Fill HTS Eligibility Screening Form  ${x.patient}`, async ({ page }) => {
  ...
  await test.step("Asserting the client relationship concepts and select the  None option", async () => {
    const elementsvalue = [];
    const clientrel1option = await page.locator("//input[@id='0cricAdulTid']");
    const clientrel2option = await page.locator("//input[@id='1cricAdulTid']");
    const clientrel3option = await page.locator("//input[@id='2cricAdulTid']");
    const clientrel4option = await page.locator("//input[@id='3cricAdulTid']");
 ...
 await test.step("Selecting  Integrated VCT strategy", async () => {
  ...
  await test.step("Selecting Mother's HIV status",async ()=> {
    if(patient.person.age < 10 ){
     const elementvalue = await page.locator("input[name=motHersHivstatusid]");
     const malnutrition = await page.locator("//input[@id='0EligiBiliTykiDsid']");
     const  failuretothrive = await page.locator("//input[@id='1EligiBiliTykiDsid']");
     const  RecurrentInfections = await page.locator("//input[@id='2EligiBiliTykiDsid']");
     const  TB = await page.locator("//input[@id='3EligiBiliTykiDsid']");
     const  noparent = await page.locator("//input[@id='4EligiBiliTykiDsid']");
   const elementcount = await elementvalue.count();
   for (var index = 0; index < elementcount; index++) {
     const element = await elementvalue.nth(index);
     const innerText = await element.getAttribute("value");
  ...
  await test.step("Selecting client ever had sex and validation the questions", async () => {
    ...
    for (var index = 0; index < elementcount; index++) {
      ...
      if (innerText == "1") {
        ...
        await expect(page.locator("input[name=newPartnerid]").first()).toBeVisible();
        await expect(
        ...
        await expect(page.locator("input[name=alcoholicSexid]").first()).toBeVisible();

        await expect(page.locator("input[name=moneySexid]").first()).toBeVisible();
        await expect(page.locator("input[name=condomBurstid]").first()).toBeVisible();
        await expect(page.locator("input[name=strangerSexid]").first()).toBeVisible();

        await expect(page.locator("input[name=knownPositiveid]").first()).toBeVisible();
      }
    }
  });
  ...
  await test.step("validating partnerhiv status and selecting hiv negative", async () => {
    const elementvalue = await page.locator("input[name=partnerHivStatusid]");
    const elementcount = await elementvalue.count();
    for (var index = 0; index < elementcount; index++) {
      const element = await elementvalue.nth(index);
      const innerText = await element.getAttribute("value");
  ...
  await test.step("validating multiple partners unprotected sex and the number", async()=>{
    const elementvalue = await page.locator("input[name=multipleSexPartnersid]");
    const elementcount = await elementvalue.count();
    for (var index = 0; index < elementcount; index++) {
      const element = await elementvalue.nth(index);
      const innerText = await element.getAttribute("value");
  ...
  await test.step("validating the traditional scarification and selecting yes",async()=>{
    const elementvalue = await page.locator("input[name=traditionalMethdsid]");
    const elementcount = await elementvalue.count();
    for (var index = 0; index < elementcount; index++) {
      const element = await elementvalue.nth(index);
      const innerText = await element.getAttribute("value");
  ...
  await test.step("Validating the GBV questions and selecting yes",async()=>{
    ...
    for (var index = 0; index < elementcount; index++) {
      ...
      if (innerText == "1065AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA") {
        ...
        expect(page.locator("input[name=sexualViolenceid]").first()).toBeVisible();
        expect(page.locator("input[name=forcedSexid]").first()).toBeVisible();
      }
    }
  })
 ...
 await test.step("Checking the PREP service and assertng currently on prep question is present",async()=>{
   await page.evaluate("document.getElementById('0prepServiceid').click()");
  
     await expect(page.locator("input[name=currentlyPrepid]").first()).toBeVisible();
 ...
 await test.step("Validating currently on  prep options and selecting yes",async()=>{
  ...
  const elementvalue = await page.locator("input[name=currentlyPrepid]");
    const elementcount = await elementvalue.count();
    for (var index = 0; index < elementcount; index++) {
      const element = await elementvalue.nth(index);
      const innerText = await element.getAttribute("value");
 ...
 await test.step("Checking the PEP service and assertng client used PEP question is present",async()=>{
  ...
  await page.evaluate("document.getElementById('0pepServiceid').click()");
  await expect(page.locator("input[name=currentlyPepid]").first()).toBeVisible();
...
await test.step("Validating currently on  prep options and selecting yes",async()=>{
 ...
 const elementvalue = await page.locator("input[name=currentlyPepid]");
   const elementcount = await elementvalue.count();
   for (var index = 0; index < elementcount; index++) {
     const element = await elementvalue.nth(index);
     const innerText = await element.getAttribute("value");
...
await test.step("Checking the STI service and asserting if the client had STI question is present",async()=>{
  ...
  await page.evaluate("document.getElementById('0stiServiceid').click()");
 
    await expect(page.locator("input[name=currentlyStiid]").first()).toBeVisible();
...
await test.step("Validating client had STI options and selecting yes",async()=>{
 ...
 const elementvalue = await page.locator("input[name=currentlyStiid]");
   const elementcount = await elementvalue.count();
   for (var index = 0; index < elementcount; index++) {
     const element = await elementvalue.nth(index);
     const innerText = await element.getAttribute("value");
 ...
 await test.step("Validating the screened for TB questions and selecting yes",async()=>{
  ...
  const elementvalue = await page.locator("input[name=screenedTBid]");
   ...
   for (var index = 0; index < elementcount; index++) {
     ...
     if (innerText == "1065AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA") {
       element.click();
       await expect(page.locator("input[name=coughDurationid]").first()).toBeVisible();
       await expect(page.locator("input[name=feverid]").first()).toBeVisible();
       await expect(page.locator("input[name=weightLossid]").first()).toBeVisible();
       await expect(page.locator("input[name=experiencingSweatsid]").first()).toBeVisible();
       await expect(page.locator("input[name=tbResultstaTuSid]").first()).toBeVisible();
 ...
 await test.step("Selecting yes for the night sweats question",async()=>{
  const elementvalue = await page.locator("input[name=experiencingSweatsid]");
    const elementcount = await elementvalue.count();
    for (var index = 0; index < elementcount; index++) {
      const element = await elementvalue.nth(index);
      const innerText = await element.getAttribute("value");
 ...
 await test.step("Fill the breastfeeding question and validating the options",async()=>{
  ...
  const elementvalue = await page.locator("input[name=breastfeedingid]");
   const elementcount = await elementvalue.count();
   for (var index = 0; index < elementcount; index++) {
     const element = await elementvalue.nth(index);
     const innerText = await element.getAttribute("value");
  ...
  const elementvalue = await page.locator("input[name=eligibleTestid]");
    ...
    for (var index = 0; index < elementcount; index++) {
      ...
      if (innerText == "1065AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA") {
        ...
        expect(page.locator("input[name=clientReferredid]").first()).toBeVisible();
...
await test.step("Generate the risk Score",async()=>{
  await page.getByRole('button', { name: /Get HIV risk category/i }).click();
