...
test('Receipt Stock Test', async ({ page }) => {
    ...
    await test.step('I should see the start new operation', async () => {
        ...
        await expect(page.getByRole('menuitem', { name: 'Receipt' })).toBeVisible();
        await page.getByRole('menuitem', { name: 'Receipt' }).click();
    });

    await test.step('I should be able to fill stock operation details', async () => {
        await page.getByPlaceholder('Choose a source').click();
        await page.getByText('Kenya Medical Supplies Authority', { exact: true }).click();
