...
test('Register a new patient', async ({page}) => {
    ...
    await test.step("then I should be able to view post to registry", async () => {
        await page.getByTestId('identifier-placeholder').isVisible();
    ...
    await test.step('And then I fill the registration form with the data in `formValues` and then click the `Submit` button', async () => {
        await patientRegistrationPage.fillPatientRegistrationForm(formValues, page);
    });
    ...
    await test.step("And I should see the newly registered patient's details displayed in the patient banner", async () => {
        await expect(page.getByText('James Doe Test')).toBeVisible();
        await expect(page.getByText(/male/i)).toBeVisible();
        await expect(page.getByText(/01-Feb-1990/i)).toBeVisible();
    ...
    await test.step("Then I should see the patient's address and contact details displayed in the patient banner", async () => {
        ...
        await expect(page.getByText(/Postal Address: 12310230/i)).toBeVisible();
        await expect(page.getByText('Village: testVillage')).toBeVisible();
        await expect(page.getByText('Sub-location: testSublocation')).toBeVisible();
        await expect(page.getByText('Village: testVillage')).toBeVisible();
        await expect(page.getByText(/contact details/i)).toBeVisible();
        await expect(page.getByText(/Telephone contact: 0755551234/i)).toBeVisible();
    });
});
...
test('Registering Client with unkown Birth date', async ({page}) => {
    ...
    await test.step("Entering Contact and demographics data", async () => {
        ...
        await page.getByLabel('Telephone contact').fill('0799652622');
        ...
        await page.getByLabel('Location (optional)', {exact: true}).click();
        await page.getByLabel('Location (optional)', {exact: true}).fill('Test-location');
        await page.getByLabel('Sub-location (optional)').click();
        await page.getByLabel('Sub-location (optional)').fill('test-village');
        await page.getByLabel('Village (optional)').click();
        await page.getByLabel('Village (optional)').fill('mera-village');
        await page.getByLabel('Landmark (optional)').click();
        await page.getByLabel('Landmark (optional)').fill('uis');
    ...
    await test.step("And I should see the newly registered patient's details displayed in the patient banner", async () => {
        await expect(page.getByText('James Deivin')).toBeVisible();
        await expect(page.getByText(/Female/i)).toBeVisible();
        await expect(page.getByText(/24 yrs/i)).toBeVisible();
        await expect(page.getByText(/OpenMRS ID/i)).toBeVisible();
    });
});
